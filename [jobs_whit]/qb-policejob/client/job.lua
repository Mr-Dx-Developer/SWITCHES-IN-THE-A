...
RegisterNetEvent("police:client:opengaragemenu", function (data)
    if PlayerJob.name == "police" then
        if not PlayerJob.onduty then return QBCore.Functions.Notify("You Are not on duty") end
...
RegisterNetEvent("police:client:opengaragemenu2", function (data)
    if PlayerJob.name == "police" then
        if not PlayerJob.onduty then return QBCore.Functions.Notify("You Are not on duty") end
        exports['qb-menu']:showHeader({
            {
        ...
        if isPointInside then
		    ...
		    garage()
                    ...
                    for k, v in pairs(Config.Locations["vehicle"]) do
                        if #(point - vector3(v.x, v.y, v.z)) < 5 then
                            currentSelection = k
