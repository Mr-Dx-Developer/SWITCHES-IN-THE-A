...
local function CarHeist()
    ...
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "carheist", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
        ...
        dispatchMessage = _U('carheist'), -- message
        job = {"LEO", "police"} -- type or jobs that will get the alerts
    })
end exports('CarHeist', CarHeist)
