...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
    ...
    if not steamId then
        deferrals.done(strings.noSteam)
        CancelEvent()
    end
    ...
    if not processQueue(discordId, deferrals, src) then
        CancelEvent()
    end
end)
...
AddEventHandler('playerDropped', function(reason)
    if Config.DiscordQueue.enabled then
        removeFromQueue(GetPlayerIdentifier(source, 3))
    end
end)
