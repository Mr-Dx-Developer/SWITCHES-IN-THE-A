...
Config.FailSafeArriveTimeout = 5 -- Time in MINS before the ambulance will skip to the next stage if it is stuck on the way to the player  -- Set To false To Disable
Config.FailSafeRunToPatientTimeout = 20 -- Time in SECONDS before the medic will be teleported to the player if they are stuck on the way to the player (on foot after arriving) -- Set To false To Disable
Config.FailSafeGetInAmbulanceTimeout = 20 -- Time in SECONDS before the player will be teleported into the ambulance if they are stuck on the way to the ambulance -- Set To false To Disable
...
Config.SpawnDistance = 1000.0 -- How far away you want the ambulance to spawn from the player
Config.SafeDistance = 69 -- Distance to check for players when spawning Ambulance - to prevent it spawning infront of people
Config.DispatchDelay = 1 -- Time in seconds before the ambulance starts driving to the location
...
Config.ArrivalDistance = 25.0 -- How close the ambulance needs to be to the player before it stops and the medic gets out
...
Config.AmbulanceDriveFlag = 262975 -- Driving style for the ambulance to get to the player -- see more here: https://vespura.com/fivem/drivingstyle/
Config.AmbulanceStopRange = 12.0 -- How close the ambulance needs to be to the player before it stops and the medic gets out


Config.DropOffSpots = {  -- Where the ambulance will take you - it should always find the closest one
  ['Pillbox Hospital'] = { dropOff = vec4(380.0058, -586.8229, 28.4184, 24.9223), respawnSpot = vec4(355.5534, -596.6674, 28.7737, 245.8017)}, 
...
Config.BlacklistAreas = {  -- Areas that are blacklisted, ambulance will NOT come to a player in these areas
  ['Pillbox Area'] = { area = vec4(380.0058, -586.8229, 28.4184, 24.9223), radius = 50.0 }, 
  ['Other Area']   = { area = vec4(1120.0826, -1517.0491, 34.6926, 86.2999), radius = 50.0 }, 
...
Config.JobToCheckAmountOnline = 2 -- If there is a more than this amount of people online with the job, you can't call a medic.


Config.PaymentType = 'bank'   -- 'cash' / 'bank'

Config.ReviveCostSuccessfulSkillCheck  = 20000      -- How much it costs to be revived if you spawn back in the hospital (if AlwaysTakeToHosptial is set to true and you pass the minigame)
Config.ReviveCostFailedSkillCheck  = 10000      -- How much it costs to be revived if you spawn back in the hospital (if you fail the minigame OR have minigame set to false)
...
function FuelEvent(ambulance)
  SetVehicleFuelLevel(ambulance, 100.0)
end
